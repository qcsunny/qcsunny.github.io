...
这其实混淆了密码学里最基本的两个概念：**签名（Signature）**和**加密（Encryption）**。签名保护的是数据的**完整性与不可篡改性**，对**保密性（Confidentiality）**毫无作用。任何人拿到一段 JWT，不需要任何密钥就能直接解开看明文；如果后端的验签逻辑稍微留有瑕疵，攻击者甚至能伪造身份，直接跨过你的防线。

本站的 [JWT 在线解码器](/security/jwt-decoder/) 与 [Base64 编解码器](/devtools/base64/) 是按照纯本地、零网络请求的原则写的。这篇文章就把 JWT 的底层三段式、历史上几次经典的漏洞踩坑史，以及防御逻辑彻底掰碎讲清楚。
