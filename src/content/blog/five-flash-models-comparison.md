...
### 5 款模型 1M Tokens 官方与折合成本对比分析表

> **折算基准**：按 2026 年 9 月 8 日实际即期汇率 1 USD ≈ 6.71 CNY 统一折算；单价均按每 100 万（1M）tokens 计费。

| 模型与计费版本 | 官方公布原币单价（每 1M tokens） | 折合 CNY 输入（未缓存） | 折合 CNY 输出 | 缓存命中成本（每 1M tokens） | 1M 输入 + 1M 输出基准 | 计费机制与成本特性 |
| --- | --- | --- | --- | --- | --- | --- |
| **GLM-5.3-Flash**<br>*(限时五折促销)* | 输入 \$0.075<br>输出 \$0.25<br>缓存命中 \$0.015 | **¥0.50** | **¥1.68** | ¥0.10（\$0.015） | **¥2.18**（\$0.325） | 限时 5 折优惠至 2026-09-09 24:00 (UTC+8) 截止；原价恢复后折合约输入 ¥1.01、输出 ¥3.36（综合 ¥4.36） |
| **MiMo-V2.5 标准版**<br>*(国内人民币渠道)* | 输入 ¥1.00<br>输出 ¥2.00<br>缓存命中 ¥0.02 | **¥1.00** | **¥2.00** | **¥0.02** | **¥3.00** | 国内直付人民币，支持国内 6% 增值税专票；免创建费且自动缓存命中低至 ¥0.02；美元渠道隐含汇率 7.14 存在剪刀差 |
| **Qwen3.8-Flash**<br>*(国内多区标准)* | 输入 ¥0.80<br>输出 ¥2.70<br>缓存命中 ¥0.10 | **¥0.80** | **¥2.70** | ¥0.10（需创建费 ¥1.25） | **¥3.50** | 北京/法兰克福/东京/弗吉尼亚同价；显式上下文缓存（创建按输入计费 ¥1.25，命中享受极低折扣）；海外区略高（约 ¥1.09 / ¥3.43） |
| **DeepSeek-V4-Flash**<br>*(非峰时·错峰)* | 输入 \$0.22<br>输出 \$0.66<br>缓存命中 \$0.007 | **¥1.48** | **¥4.43** | ¥0.05（\$0.007） | **¥5.90**（\$0.88） | 周末全天及工作日夜间与午休非峰时（直降 50%）；无需显式创建缓存，自动命中极低单价；官方提供 2500 极高并发支持 |
| **DeepSeek-V4-Flash**<br>*(工作日峰时)* | 输入 \$0.44<br>输出 \$1.32<br>缓存命中 \$0.014 | **¥2.95** | **¥8.86** | ¥0.09（\$0.014） | **¥11.81**（\$1.76） | 工作日 09:00–12:00 与 14:00–18:00（北京时间）；批处理任务强烈建议错峰调度节省 50% API 支出 |
| **Hy4 preview**<br>*(TokenHub 托管)* | 输入 \$0.834<br>输出 \$2.501<br>缓存命中 \$0.042 | **¥5.60** | **¥16.78** | ¥0.28（\$0.042） | **¥22.38**（\$3.335） | 770B 庞大参数主干的推理硬件成本直接反映在托管单价；长线大吞吐调用推荐利用其 Apache-2.0 权重自行私有部署 |

### 典型 1M 上下文长任务单次费用实测测算

在实际工程中，长上下文场景通常表现为“输入极其庞大、输出相对精炼”。假定单次超长文档分析、代码库审查或多模态长视频总结任务消耗 **1M 未缓存输入 tokens + 10K 输出 tokens**（即 100 万输入 + 1 万输出）：

- **GLM-5.3-Flash（限时五折）**：输入 ¥0.50 + 输出 ¥0.017 ≈ **¥0.52**（恢复标价后约 **¥1.04**）
- **Qwen3.8-Flash（国内区）**：输入 ¥0.80 + 输出 ¥0.027 ≈ **¥0.83**
- **MiMo-V2.5 标准版（人民币直付）**：输入 ¥1.00 + 输出 ¥0.020 ≈ **¥1.02**
- **DeepSeek-V4-Flash（非峰时）**：输入 ¥1.48 + 输出 ¥0.044 ≈ **¥1.52**（工作日峰时约为 **¥3.04**）
- **Hy4 preview（TokenHub 托管）**：输入 ¥5.60 + 输出 ¥0.168 ≈ **¥5.77**

若同一批 1M tokens 文档需要进行 **10 轮连续提问与多轮 Agent 循环（开启 Context Caching，假设输入缓存命中率 100%）**，总费用将呈现显著分化：
- **MiMo-V2.5**：首次未命中输入 ¥1.00 + 后续 9 次缓存命中 (9 × 1M × ¥0.02 = ¥0.18) + 10 次输出 (0.1M × ¥2.00 = ¥0.20) = **¥1.38**；
- **DeepSeek-V4-Flash（非峰时）**：首次未命中输入 ¥1.48 + 后续 9 次缓存命中 (9 × 1M × ¥0.047 ≈ ¥0.42) + 10 次输出 (0.1M × ¥4.43 = ¥0.44) ≈ **¥2.34**（峰时约 **¥4.68**）；
- **Qwen3.8-Flash**：显式缓存创建费 ¥1.25 + 后续 9 次命中 (9 × 1M × ¥0.10 = ¥0.90) + 10 次输出 (0.1M × ¥2.70 = ¥0.27) = **¥2.42**；
- **Hy4 preview**：首次未命中输入 ¥5.60 + 后续 9 次缓存命中 (9 × 1M × ¥0.282 ≈ ¥2.54) + 10 次输出 (0.1M × ¥16.78 = ¥1.68) ≈ **¥9.82**。
