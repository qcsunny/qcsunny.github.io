...
[Markdown 实时预览](/text/markdown-preview/)这个工具里有两套完全不同的东西：Markdown 解析是从零手写的，一行第三方代码都没有；数学公式排版则是把 KaTeX 打包进了自己的产物。这篇文章讲清楚这条界线划在哪里、为什么划在那里，以及手写解析器里最容易写错的那一类 bug——**规则之间互相钻进对方的领地**。写这篇文章的过程中又抓到一个同源的、已经上线的实例，也一并记在下面。
...
- **[Markdown 实时预览](/text/markdown-preview/)**：本文拆解的这份实现，双栏实时预览、`$inline$` 与 `$$display$$` 公式、导出 .md 与自包含 .html；
