...
export const TOOL_KEYWORDS: Record<string, string> = {
	...
	'prime-factorization': '质因数分解 质因数 质数 素数 分解质因数 约数 约数个数 算术基本定理 prime factorization factors prime numbers divisors pollard rho miller rabin',
	'combinatorics': '组合数 排列数 阶乘 排列组合 nCr nPr combinations permutations factorial combinatorics bigInt',
	'number-base-converter': '进制转换 二进制 八进制 十进制 十六进制 base converter binary octal decimal hex radix',
	'unix-timestamp': '时间戳 unix时间戳 时间转换 毫秒 秒 时区 timestamp unix epoch datetime convert',
	'cron-expression-parser': 'cron 定时任务 cron表达式 cron解析 定时器 crontab schedule parser expression',
	'regex-tester': '正则表达式 正则 正则测试 正则匹配 regex regular expression tester pattern match',
	'css-px-rem-converter': 'px rem 换算 像素 根字号 rem转px px转rem css px rem converter em',
	'text-diff': '文本对比 文本差异 diff 差异对比 代码对比 text diff compare difference',
	'hash-generator': '哈希 散列 sha256 sha1 md5 哈希值 hash generator checksum digest',
