...
export const TEXT_TOOLS: ToolEntry[] = [
	...
	},
	{
		...
		kind: 'text',
		config: {
			def: '| ID | Module | Category | Core technology & features | Latency | Status | Concurrency |\n|:---:|:---|:---:|:---|---:|:---:|---:|\n| 101 | **API Gateway** | Microservice | OAuth2.0, `JWT` auth, *multi-active failover* | 12ms | 🟢 Ready | 50,000 QPS |\n| 102 | **Vector DB** | Vector store | Milvus, HNSW index, hybrid search | 35ms | 🟢 Ready | 12,000 QPS |\n| 103 | **Task Scheduler** | Batch | Distributed sharding, ~~legacy single-node scheduler~~, CRON | 1,250ms | 🟡 Degraded | 1,500 Jobs/m |\n| 104 | **Document OCR** | Compute-bound | Pure WebAssembly offline OCR, table extraction | 480ms | 🟢 Ready | 800 Pages/m |\n| 105 | **Event Bus** | Message broker | Kafka cluster, `ack=all`, Exactly-Once | 5ms | 🟢 Ready | 120,000 QPS |\n| 106 | **Cache Proxy** | Cache tier | Redis cluster, `get | set` batch ops, shard warm-up | 2ms | 🟢 Ready | 200,000 QPS |\n',
			defZh: '| ID | 模块名称 / Module | 架构分类 | 核心技术栈与特性 | 响应时延 | 运行状态 | 并发能力 |\n|:---:|:---|:---:|:---|---:|:---:|---:|\n| 101 | **API Gateway** | 微服务 | OAuth2.0、`JWT` 鉴权、*多活容灾* | 12ms | 🟢 Ready | 50,000 QPS |\n| 102 | **Vector DB** | 向量库 | Milvus、HNSW 索引、混合检索 | 35ms | 🟢 Ready | 12,000 QPS |\n| 103 | **Task Scheduler** | 批处理 | 分布式分片、~~旧单机调度~~、CRON | 1,250ms | 🟡 Degraded | 1,500 Jobs/m |\n| 104 | **Document OCR** | 计算密集 | 纯 WebAssembly 离线识别、表格提取 | 480ms | 🟢 Ready | 800 Pages/m |\n| 105 | **Event Bus** | 消息中间件 | Kafka 集群、`ack=all`、Exactly-Once | 5ms | 🟢 Ready | 120,000 QPS |\n| 106 | **Cache Proxy** | 缓存层 | Redis 集群、`get | set` 批量操作、分片预热 | 2ms | 🟢 Ready | 200,000 QPS |\n',
