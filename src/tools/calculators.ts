...
const piCalculator: FormConfig = {
	...
	introZh: '采用分阶混合引擎（低位数 Machin 秒开，1.5 万位以上自动切换 Chudnovsky 楚德诺夫斯基超高阶级数 + 二进制拆分）计算圆周率 π 至 100 万位（1,000,000），包含 CPU 性能检测与分级预设。',
	fields: [
		{
			id: 'digits',
			...
			hintZh: '支持 1 到 1,000,000 位（100 万位）高精度计算（混合引擎：Machin + Chudnovsky）。',
			presets: [
...
export const CALCULATOR_TOOLS: ToolEntry[] = [
	...
	},
	{
		...
		descriptionZh: '采用分段混合引擎（低位数 Machin 秒开，1.5 万位以上自动切换 Chudnovsky 超高阶级数与二进制拆分）实时计算圆周率 π 至 100 万位（1,000,000）。',
		kind: 'form',
		config: piCalculator,
	},
	{
