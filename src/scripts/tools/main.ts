...
void (async () => {
	...
	// (qr, color, json, jsonschema, meta, fractal, imgfilter, gridgen, flexgen, xlsxanalyzer, pdftoolkit, pdfocr, sql, jwt, url, xml, css, html, markdown) renders standalone.
	...
	switch (kind) {
		...
		case 'json':
			...
			break;
		case 'jsonschema':
			await import('./jsonschema').then((m) => m.initJsonSchema(host));
			break;
		case 'meta':
		...
		case 'imgfilter':
			...
			break;
		case 'gridgen':
			await import('./csslayout').then((m) => m.initGridGen(host));
			break;
		case 'flexgen':
			await import('./csslayout').then((m) => m.initFlexGen(host));
			break;
		case 'xlsxanalyzer':
			await import('./xlsxui').then((m) => m.initXlsxAnalyzer(host));
			break;
		case 'pdftoolkit':
			await import('./pdftoolkit-ui').then((m) => m.initPdfToolkit(host));
			break;
		case 'pdfocr':
			await import('./pdfocr').then((m) => m.initPdfOcr(host));
			break;
		case 'sql':
