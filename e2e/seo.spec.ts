...
test('the favicon set covers raster consumers, not just the SVG path', async ({ page, request }) => {
	// `rel="icon" type="image/svg+xml"` is crisp in the Chromium and Safari tab
	// strips, but everything that draws an icon as a raster — a Windows app
	// shortcut, an Explorer thumbnail, an iOS home-screen icon — asks for the
	// .ico or the apple-touch-icon, and neither may be a 16/32px stub: scaling
	// one of those two up is exactly the aliasing the site used to show. The
	// frames are derived from public/favicon.svg by scripts/make-icons.mjs.
	await page.goto('/blog/');

	const links = await page.$$eval('link', (els) =>
		els.map((el) => ({ rel: el.rel, type: el.type, href: el.getAttribute('href') })),
	);
	const byHref = (href: string) => links.find((l) => l.href === href);

	expect(byHref('/favicon.svg')!.type).toBe('image/svg+xml');
	expect(byHref('/favicon.ico')!.rel).toBe('icon');
	expect(byHref('/apple-touch-icon.png')!.rel).toBe('apple-touch-icon');

	// Read the ICO directory rather than trusting the file exists: the regression
	// here was a valid, non-empty ICO holding two small frames.
	const ico = await (await request.get('/favicon.ico')).body();
	const count = ico.readUInt16LE(4);
	const sizes = Array.from({ length: count }, (_, i) => ico.readUInt8(6 + i * 16) || 256);
	expect(sizes).toContain(16); // the tab strip
	expect(Math.max(...sizes)).toBeGreaterThanOrEqual(256); // Explorer, app shortcuts

	// iOS asks for 180x180; PNG IHDR carries the size big-endian at bytes 16 and 20.
	const touch = await (await request.get('/apple-touch-icon.png')).body();
	expect(touch.readUInt32BE(16)).toBe(180);
	expect(touch.readUInt32BE(20)).toBe(180);

	// The icon URLs never change but their content is regenerated by
	// make-icons.mjs, so they get a day of browser cache — not immutable, or a
	// redesigned mark would sit in caches for a year.
	...
	for (const href of ['/favicon.svg', '/favicon.ico', '/apple-touch-icon.png']) {
		expect(rules, href).toMatch(new RegExp(`^${href.replace(/\./g, '\\.')}$`, 'm'));
	}
	expect(rules).toMatch(/\/apple-touch-icon\.png\n\s*Cache-Control: public, max-age=86400/);
});
...
test('_headers ships immutable caching for content-hashed assets', async () => {
	const raw = await readFile(new URL('../dist/_headers', import.meta.url), 'utf8');
	// `#` lines are commentary and may well say the word "immutable" while
	// explaining why something is NOT immutable. Strip them so these assertions
	// read the rules rather than the prose.
	const rules = raw
		.split('\n')
